...
const LanguagePageNew = ({ languages }) => {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {filteredLanguages.map((lang) => (
                        <button
                            ...
                            className="flex items-center gap-4 p-4 rounded-2xl bg-white dark:bg-[rgb(var(--color-bg-card))] border border-gray-100 dark:border-gray-800 hover:border-[rgb(var(--color-primary))] hover:scale-[1.02] active:scale-95 transition-all shadow-sm group"
                        >
                            {/* Le "Drapeau" ou Symbole Visuel */}
                            <div className="w-12 h-12 rounded-xl bg-[rgb(var(--color-bg-pills))] flex items-center justify-center overflow-hidden group-hover:bg-[rgb(var(--color-primary))] transition-colors">
                                {lang.lang_flag_icon ? (
                                    <img
                                        ...
                                        onError={(e) => {
                                            // Optionnel : remplace l'image par un emoji globe si le lien est mort
                            ...
                            {/* Textes (Double écriture pour l'inclusivité) */}
                            <div className="text-left">
                                <p className="text-sm font-bold text-gray-400 uppercase tracking-widest leading-none mb-1">
                                    {lang.lang_name_fr}
                                </p>
                                <p className="text-lg font-bold text-[rgb(var(--color-text-main))]">
...
export default LanguagePageNew;